...
processMessage: function(_message, source, callback) {
		...
		getUser(user).then(function(dbUser) {
				...
				.then(function(messageResponse) {
						...
						if (responseContext.hasOwnProperty('api')) {
							...
							switch (responseContext.api.RUN) {
								case 'LPA':
									...
									return getLoopLength(responseContext.api.ban)
										.then(function(loopLength) {
											extend(responseContext.api, loopLenght);
					...
					break;
					case 'CRM':
						debug('Fake CRM response'); responseContext.api.OLDBILLAMT = "$55"; responseContext.api.NEWBILLAMT = "$48"; delete responseContext.api.RUN;
						...
						break;
					case 'BBNMS':
						debug('Fake BBNMS response'); responseContext.api.ORDNMBR = "2346771608A"; delete responseContext.api.RUN;
