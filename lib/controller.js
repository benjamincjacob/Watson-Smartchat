	...
	processMessage: function(_message, source, callback) {
		...
		getUser(user).then(function(dbUser) {
			...
			.then(function(messageResponse) {
					...
					if (responseContext.hasOwnProperty('api')) {
						...
						switch (responseContext.api.RUN) {
							case 'LPA':
								...
								getLoopLength(responseContext.confirmed.BAN)
								...
								debug('5A. Conversation second send: %s.', JSON.stringify(messageResponse, null, 2));
								return sendMessageToConversation(message);
								break;
							case 'CRM':
				...
				.then(function(messageResponse) {
					debug('5A. Conversation second response: %s.', JSON.stringify(messageResponse, null, 2));
